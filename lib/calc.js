...
export const DEFAULTS = {
  ...
  submissionDate: "", // YYYY-MM-DD (when the bid was submitted)
  ...
  // Overtime: when otOn, a share of FIELD hours are worked at OT (1.5x). We add
  // only the PREMIUM (the extra 0.5x) into labor cost, so it flows through tools,
  // contingency, and margin like any other labor. Off by default; 10% when on.
  otOn: false,
  otPct: 0.10,
  otHoursPerWeek: 44, // convenience input; (hrs-40)/hrs -> otPct
...
export function computeEstimate(i) {
  ...
  // Overtime premium: fieldMH x OT% x 0.5 x loadedRate (the extra half over straight
  // time). Only the placement (field) hours can be OT — mobilization is not.
  const otPremium = i.otOn ? fieldMH * (i.otPct || 0) * 0.5 * loadedRate : 0;
  const directLabor = totalMH * loadedRate + otPremium; // B31 + OT premium
  ...
  // OT's contribution to the bid in ¢/lb: gross the premium up through the same
  // tools/contingency/margin path the bid uses, then express per lb.
...
hasCostBasis: false, recommendedRate: null, rateUnit: "", qtyLabel: "", otPremium: 0,
  };
...
// ---------------------------------------------------------------------------
// Out-of-Town (Travel) Cost Add-On — LABOR-STACK-INDEPENDENT
// ---------------------------------------------------------------------------
// Hotel + Fuel + Subsistence. Travel does NOT pass through the labor
// burden/tools/contingency multipliers, and does NOT inherit the placement
// target margin. It carries its OWN optional markup (default 12%). The result
// converts to ¢/lb on the project's rebar weight and folds into the bid as a
// visible add-on. Modular: add more travel components later without touching
// the rebar math.
